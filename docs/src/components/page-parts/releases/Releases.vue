...
export default {
  ...
  setup () {
    ...
    const currentPackage = ref('QIconPicker')
    const versions = ref({})
    ...
    function queryReleases (page = 1) {
      ...
      xhrQuasar.addEventListener('load', function () {
        ...
        // sort by date
        packages.value.QIconPicker.sort((a, b) => {
      ...
      xhrQuasar.open('GET', `https://api.github.com/repos/quasarframework/quasar-ui-qiconpicker/releases?page=${ page }&per_page=100`)
      xhrQuasar.send()
    }
