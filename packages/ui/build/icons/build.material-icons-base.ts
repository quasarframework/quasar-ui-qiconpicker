...
function run(googleIcons, theme) {
  ...
  const whiteListed = {
    ...
    outlined: ['motorcycle', 'pie_chart_outlined'],
    round: [
      'info_outline',
      'label_outline',
      'lightbulb_outline',
    ...
    sharp: ['label_outline', 'lightbulb_outline', 'lock_outline', 'motorcycle'],
  }
  ...
  try {
    ...
    console.warn(`${blue('[icon]')} ${green(name + ':')} skipping old metadata seed`)
  }
  ...
  writeFile(path.resolve(__dirname, outputLocation), output).catch((exception) =>
